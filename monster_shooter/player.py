            ...
            if player_pos[0] >= monster_pos[0] and player_pos[0] <= monster_pos[0] + monster_pic.get_rect().size[0]:
                if player_pos[1] >= monster_pos[1] and player_pos[1] <= monster_pos[1] + monster_pic.get_rect().size[1]:
                    if monster_type == 0:
                        ...
                        if player_pos[0] >= weak_point_pos[0] and player_pos[0] <= weak_point_pos[0] + weak_point_size:
                            if player_pos[1] >= weak_point_pos[1] and player_pos[1] <= weak_point_pos[1] + weak_point_size:
                                return self.strength * 3
        return 0
    
    def shoot_air(self, shoot_flag):
        if self.ammo > 0 and shoot_flag:
            self.shoot_sfx.play()
            self.ammo -= 1
